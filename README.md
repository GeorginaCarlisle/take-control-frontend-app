...
| # 6 | The error handling in the create form is not working as expected. When the form is submitted with a blank title field the following is logged to the browser console "Uncaught (in promise) TypeError: _errors$title2.value is not a function" | Checked code and compared with the focus create form which does handle errors as expected. Nothing noticed. Troubleshooted with chatgpt, this threw up no solutions. Request for help in slack and another set of eyes to look at the code. | |
